...
// I also need to make sure the selects look exactly like the inputs. 
// Adding Chevron down icon to selects by a CSS trick or keeping appearance-none but using a wrapper.
// Actually standard select in Tailwind looks okay with bg-[#1C1C1E] border border-white/5. Let's just use the default <select> with standard Tailwind styling.
// We'll replace the grid part.
...
const newGrid = `              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8 mb-8">
                ...
                <div>
                  ...
                  <select 
                    className="flex h-10 w-full rounded-md border border-border bg-muted px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
...
const startMarker = `              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">`;
const endMarker = `            </div>
